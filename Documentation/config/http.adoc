...
http.connectTimeout::
	Specifies how long, in seconds, to allow the HTTP connection phase
	to complete before aborting the transfer. This includes DNS,
	TCP, and TLS connection setup. Can be overridden by the
	`GIT_HTTP_CONNECT_TIMEOUT` environment variable.
